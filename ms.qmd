...
# A) L. deli
    #a.1) Mass 
plot_mass <- ggplot(deli_stressor, aes(x = mass, fill = interaction(as.factor(temp),as.factor(cort), sep="_"))) +
...
# I will both distributions with saphiro.test and lat_to_move with a qqplot
mass_normal_test<-by(deli_stressor$mass, interaction (deli_stressor$cort, deli_stressor$temp, deli_stressor$Day), function(x) shapiro.test((x)))
...
# Once checked the distributions, I will transform lat_to_move with log(x+1) to make it normal


# B) L. guich
    #b.1) Mass 
plot_mass <- ggplot(guich_stressor, aes(x = mass, fill = interaction(as.factor(temp),as.factor(cort), sep="_"))) +
  geom_histogram(position = "identity", alpha = 0.5, bins = 30, color = "black") +
  scale_fill_manual(values = c("28_A" = "red", "23_A" = "blue", "28_B" = "salmon", "23_B" = "lightblue")) +
  labs(x = "Mass", y = "Density", fill = "Temperature ~ CORT") +
  theme_minimal() +
...
# I will both distributions with saphiro.test
